...
export function getPerson() { //请求推荐歌单数据
  return axios.get(`${host}/personalized`).then(res => {
...
export function getRecommend() { //每日歌单
  return axios.get(`${host}/recommend/resource`).then(res => {
    return res.data
  })
}

export function getSearch(keywords) { //搜索
  return axios.get(`${host}/search?keywords=${keywords}`).then(res => { //result
    return res.data
  })
}

export function getUrl(id) { //获取歌曲播放文件
  return axios.get(`${host}/song/url?id=${id}`).then(res => {
    return res
  })
}

export function getDetail(id) { //获取歌曲详情
  return axios.get(`${host}/song/detail?ids=${id}`).then(res => {
    return res
  })
}

export function getLyric(id) { //获取歌词
  return axios.get(`${host}/lyric?id=${id}`).then(res => {
    return res
  })
}

export function get(id) { //获取
  return axios.get(`${host}/related/allvideo?id=${id}`).then(res => {
    return res
  })
}

export function getVideo() { //获取视频
  return axios.get(`${host}/top/mv?limit=10`).then(res => {
    return res.data
  })
}
//请求接口
